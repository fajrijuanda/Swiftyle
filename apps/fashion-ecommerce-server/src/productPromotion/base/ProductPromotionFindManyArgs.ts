/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductPromotionWhereInput } from "./ProductPromotionWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ProductPromotionOrderByInput } from "./ProductPromotionOrderByInput";

@ArgsType()
class ProductPromotionFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ProductPromotionWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ProductPromotionWhereInput, { nullable: true })
  @Type(() => ProductPromotionWhereInput)
  where?: ProductPromotionWhereInput;

  @ApiProperty({
    required: false,
    type: [ProductPromotionOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ProductPromotionOrderByInput], { nullable: true })
  @Type(() => ProductPromotionOrderByInput)
  orderBy?: Array<ProductPromotionOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ProductPromotionFindManyArgs as ProductPromotionFindManyArgs };
